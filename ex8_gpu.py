...
if __name__ == '__main__':
    ...
    # Parse start and end indices for batch processing
    start_idx = int(sys.argv[1]) if len(sys.argv) > 1 else 0
    end_idx = int(sys.argv[2]) if len(sys.argv) > 2 else 1
    
    with open(join(LOAD_DIR, 'building_ids.txt'), 'r') as f:
        # Slice the list to only process the assigned chunk
        building_ids = f.read().splitlines()[start_idx:end_idx]

    N = len(building_ids)
    print(f"Processing {N} buildings (from index {start_idx} to {end_idx}) using Numba JIT...")
